...
class CartItemsDAO @Inject() (protected val dbConfigProvider: DatabaseConfigProvider) extends HasDatabaseConfigProvider[JdbcProfile] {
  ...
  def all(implicit ec: ExecutionContext): Future[Seq[CartItemREST]] = {
    ...
    futureCartItems.map(
      _.map {
        z => CartItemREST(orderId = z.orderId, productId = z.productId, amount = z.amount, typename = z.typename)
      }.toList
    )
  }
  ...
  def getCartItemById(id: Int): Future[CartItem] = {
    ...
    futureCartItem.map(
      _.map {
        z => CartItem(id = z.id, orderId = z.orderId, productId = z.productId, amount = z.amount, typename = z.typename)
      }.toList.head
    )
  }
  ...
  private class CartItemsTable(tag: Tag) extends Table[CartItem](tag, "TCartItem"){
    ...
    def * = (id, productId, orderId, amount, typename) <> ((models.CartItem.apply _).tupled, models.CartItem.unapply _)
  }
}
